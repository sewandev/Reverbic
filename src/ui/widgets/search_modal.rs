...
use crate::app::{IntegrationView, SearchMode, SettingItem, SpotifyAuthStatus, SpotifyField, settings_items};
...
impl SearchModalWidget<'_> {
    ...
    fn render_integ_service_list(&self, area: Rect, lx: u16, lw: u16, buf: &mut Buffer) {
        let mut y = area.y + 1;
    ...
    fn render_integ_spotify_detail(&self, area: Rect, lx: u16, lw: u16, buf: &mut Buffer) {
        let mut y = area.y + 1;
        if y >= area.bottom() { return; }
    ...
    fn render_integ_spotify_userpass(
