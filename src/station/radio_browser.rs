...
impl From<RadioBrowserStation> for DynamicStation {
    fn from(rb: RadioBrowserStation) -> Self {
        ...
        Self {
            key: rb.stationuuid,
            name: rb.name,
            url: if rb.url_resolved.is_empty() {
                rb.url
            } else {
                rb.url_resolved
            },
            bitrate_kbps: if rb.bitrate > 0 {
                Some(rb.bitrate as u16)
            } else {
                None
            },
            country: rb.country,
...
pub async fn fetch_station_details_by_name(name: &str) -> Option<StationDetails> {
    ...
    fetch_first(&format!(
        "/json/stations/search?name={encoded}&limit=1&hidebroken=true"
    ))
    .await
}
                ...
                Ok(body) => match serde_json::from_str::<Vec<RadioBrowserStation>>(&body) {
                    Ok(stations) => {
                        let dynamic: Vec<DynamicStation> = stations
                            ...
                            .collect();
                        tracing::info!(
                            "Radio Browser [{param}={value}]: {} stations",
                            dynamic.len()
                        );
                        return Some(dynamic);
                    }
