...
- Animated status dot on the active tab: a softly pulsing dot next to the current tab shows the health of that source — intense green when everything works, amber on [Spotify] in Remote mode with no device, red on [YouTube] when the cookies session expired and gray when not connected or configured
